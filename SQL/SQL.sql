...
CREATE OR REPLACE FUNCTION logs.find_value(p_value text) 
RETURNS integer AS $$
DECLARE 
        id integer;
        hh integer;
...
BEGIN    
	...
	vals := array[]::text[];
        ...
        LOOP        
		    ...
		    RETURN QUERY SELECT rec.value, rec.values_id;      
            END IF;  
...
BEGIN  
		...
		all_vals := array[]::int[];
               ...
               -- FOREACH attr IN ARRAY pck.attributes
				...
				all_vals := array_append(all_vals, values_id);
                ...
                -- dodaj log
                INSERT INTO logs.logs(logs_id, server_date, client_date, expire, uid, level, kind, source, all_values)
                VALUES (lid, pck.server_date, pck.client_date, pck.expire, pck.uid, pck.level, fkind, fsource, all_vals);
